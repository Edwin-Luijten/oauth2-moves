...
class MovesTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->provider = new \League\OAuth2\Client\Provider\Moves(
            [
    ...
    public function testGetAuthorizationUrl()
    {
        $url = $this->provider->getAuthorizationUrl();
        $uri = parse_url($url);
        $this->assertEquals('/oauth/v1/authorize', $uri['path']);
    }
    ...
    public function testGetBaseAccessTokenUrl()
    {
        ...
        $uri    = parse_url($url);
        $this->assertEquals('/oauth/v1/access_token', $uri['path']);
    }
    ...
    public function testUserData()
    {
        ...
        $userResponse->shouldReceive('getBody')->andReturn(
            '{"userId": ' . $userId . ', "profile": {"caloriesAvailable": "' . $caloriesAvailable . '", "firstDate": "' . $firstDate . '"}}'
        );
        ...
        $this->assertEquals($userId, $user->toArray()['userId']);
        $this->assertEquals($firstDate, $user->getFirstDate());
        $this->assertEquals($firstDate, $user->toArray()['profile']['firstDate']);
        $this->assertEquals($caloriesAvailable, $user->getCaloriesAvailable());
        $this->assertEquals($caloriesAvailable, $user->toArray()['profile']['caloriesAvailable']);
    }
